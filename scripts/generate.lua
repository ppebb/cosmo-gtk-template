...
--- @param path string
--- @return string
local function fname(path) return path:match("([^/]+)$") end
...
local copied_headers = {}
local function copy_headers(lib_name, source, dest, force)
    table.insert(copied_headers, string.format("%s;%s", source:gsub("%*", ""), dest))
...
local defs = {
    ...
    gir = {
        opts = {
            ...
            lib_headers = { "girepository/girepository.h", "girepository/girffi.h" },
        },
        stubs = {
            {
                headers = {
...
local script_path = get_script_dir()

if not script_path then
    fprintf(io.stderr, "Unable to determine script path, exiting...\n")
    os.exit(1, true)
end

local stubs_root = path_combine(script_path, "..", "stubs/")
local extra_headers_dir = path_combine(stubs_root, "headers")

if not file_exists(stubs_root) then
    assert(lfs.mkdir(stubs_root))
end

if not file_exists(extra_headers_dir) then
    assert(lfs.mkdir(extra_headers_dir))
end

-- Templates
local stub_c_path = path_combine(stubs_root, "stub.c-template")
local stub_h_path = path_combine(stubs_root, "stub.h-template")
...
for lib_name, lib in pairs(defs) do
    local stub_dir = path_combine(stubs_root, lib_name .. "-stub")
    ...
    for _, stub_spec in ipairs(lib.stubs) do
        ...
        local function process_entry(full_path)
            local function process_dir(path)
                fprintf(io.stdout, "Processing directory %s\n", path)
                for entry_inner in lfs.dir(path) do
                    if entry_inner ~= "." and entry_inner ~= ".." then
                        process_entry(path_combine(path, entry_inner))
                    end
                end
            end
            ...
            if attr.mode == "directory" and not matches_any(full_path, stub_spec.skip_dirs) then
                process_dir(full_path)
            elseif attr.mode == "file" and not matches_any(full_path, stub_spec.skip_files) then
                process_header(full_path)
            end
        end

        for _, path in ipairs(stub_spec.headers) do
            process_entry(path)
        end

        for _, header in ipairs(stub_spec.extra_headers or {}) do
            copy_headers(lib_name, header, path_combine(extra_headers_dir, fname(header)))
        end
    end
...
file_write(
    path_combine(script_path, "header_map.sh"),
    ('export map=(\n    "' .. tbl_join(copied_headers, '"\n    "') .. '"\n)')
)
